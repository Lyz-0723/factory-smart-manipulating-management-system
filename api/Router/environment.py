...
from Repository.EnvironmentCRUD import get_environment_record, get_specific_environment_record, create_environment_record, delete_specific_environment_record
...
async def get_spec_env_record(env_record_id: int, current_user: Annotated[GetUser, Depends(get_current_user)]) -> GetEnviromentRecord:
  """The endpoing of getting specific environment record"""
  if not current_user.is_admin:
    raise action_forbidden
...
@router.post("/")
async def create_env_record(env_record: BaseEnvironmentRecord, current_user: Annotated[GetUser, Depends(get_current_user)]) -> None:
...
async def delete_spec_env_record(env_record_id: int, current_user: Annotated[GetUser, Depends(get_current_user)]) -> None:
